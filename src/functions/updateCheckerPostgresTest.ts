...
export const checkForUpdates = async () => {
  ...
  cron.schedule('* * * * *', async () => {
    ...
    try {
      ...
      const { rows: updates } = await postgresClient.query(
        postgresUpdateQuery,
        [lastCheckTimestamp]
      );
      ...
      if (updates.length !== 0) {
        const mongoCString = 'mongodb://127.0.0.1:27017/mongots';
        await mongoConnect(mongoCString);

        console.log(mongoCString);
        ...
        // Read json files
        const orgPath = await readFileSync(
          'C:/Users/Public/orgdataupdate.json',
          {
            encoding: 'utf8',
          }
        );
